...
Media_Cookie=$(curl -s --retry 3 --max-time 10 "https://raw.githubusercontent.com/feilongluo/check/main/cookies.txt")
IATACode=$(curl -s --retry 3 --max-time 10 "https://raw.githubusercontent.com/feilongluo/check/main/reference/IATACode.txt")
...
local_ipv6_asterisk=$local_ipv6
...
function MediaUnlockTest_NetflixCDN() {
    ...
    local IATACode2=$(curl -s --retry 3 --max-time 10 "https://raw.githubusercontent.com/feilongluo/check/main/reference/IATACode2.txt" 2>&1)
...
function Goodbye() {
    echo -e "${Font_Purple}本次测试已结束，感谢使用此脚本 ${Font_Suffix}"
}
